...
2. (Opsional) Bersihkan *file* kompilasi sebelumnya:

   ```bash
   make clean
   ```
3. Lakukan kompilasi *source code*:

   ```bash
   make all
   ```
4. Jika kompilasi berhasil, *executable file* `voxelizer.exe` dan `viewer.exe` akan terbentuk di dalam folder `bin` direktori proyek.

## 4. Cara Menjalankan dan Menggunakan Program

### Menjalankan Voxelizer
...
```bash
./bin/voxelizer.exe test/pumpkin.obj 6 test/pumpkin-voxel.obj
```
